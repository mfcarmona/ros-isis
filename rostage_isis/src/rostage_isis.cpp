...
StageNode::SubscribeModels()
{
  ...
  //ROS_INFO("[new_stage_isis] we have  %d position models and %d laser models",this->positionmodels.size(),this->lasermodels.size() );
		...
		if(this->positionmodels[r])
		{
			...
			for (size_t s = 0; s < this->lasermodels.size(); s++)
			{
				if(this->lasermodels[s])
				{
					...
					{
							//ROS_INFO("[new_stage_isis] el laser model %d pertenece al robot %d", s,r );						
						//if(new_robot.lasermodels.size() == 0){ 
						    //ROS_INFO("[new_stage_isis] adding laser publisher \"%s\" to robot %d",mapLaserName(LASER_SCAN,new_robot.lasermodels.size(),r),r);
						...
						//ROS_INFO("[new_stage_isis] el laser model %d NO! pertenece al robot %d", s,r );						
					}
				}
				else
				{
				...
				if(this->rangermodels[s])
				{
					...
					if(rangermodels[s]->Parent()==positionmodels[r])
					{
						//if(new_robot.rangermodels.size() == 0) 
						  //ROS_INFO("[new_stage_isis] adding range publisher %s to robot %d",mapName(RANGER_SCAN,r),r);
						  new_robot.range_pubs_.push_back(n_.advertise<sensor_msgs::Range>(mapName(RANGER_SCAN,r), 10));
						//new_robot.range_pub_ = n_.advertise<new_stage_isis::Range>(mapName(RANGER_SCAN,r), 10);
					
						//range_pubs_.push_back(n_.advertise<new_stage_isis::Range>(mapName(RANGER_SCAN,r), 10));
	...
	for (size_t r = 0; r < this->positionmodels.size(); r++)
	{
	  ...
	  for (size_t s = 0; s < this->robots[r].lasermodels.size(); s++)
	  {
		...
		if(samples.size())
		{
		  ...
		  {
		  	char ind[3];
		  	sprintf(ind, "_%d", (int) s);
		  	frame_id.append(ind);
		  }
		  laserMsg.header.frame_id = frame_id;
		  laserMsg.header.stamp = sim_time;
		  //ROS_DEBUG("[new_stage_isis] robot %d publishing on laser %d",r,s);		  
		...
		for (size_t s = 0; s < this->robots[r].rangermodels.size(); s++)
		{
			...
			if(sensors.size())
			{
				...
				for(unsigned int i=0;i<sensors.size();i++)
				{
					...
					ROS_DEBUG("[rostage_isis] robot %d publishing on range sensor %d",(int) r,(int) s);
